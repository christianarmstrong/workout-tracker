    ...
    return (
        <>
            ...
            <div className="col-span-9 grid grid-cols-9 items-stretch mb-2 min-h-[4rem] gap-x-2">
                <div onClick={() => router.push(`/exercises/log?id=${encodeURIComponent(id)}&name=${encodeURIComponent(name)}&reps=${encodeURIComponent(reps)}&sets=${encodeURIComponent(sets)}`)} className={type=="none" ? "col-span-9 h-full" : "col-span-7 h-full"}>
                ...
                {type!="none" ? <div onClick={handleButtonClick} className="col-span-2 h-full flex items-stretch">
                    <Button className="text-sm outline rounded-sm shadow-md p-4 w-full h-full flex items-center"> { type=="remove" ? <Minus/> : type=="add" ? <Plus/> : <Pencil/>} </Button>
                </div> : null}
            </div>
        </>
    );
